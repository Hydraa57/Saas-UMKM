...
- [x] **Bisa dipasang ke layar depan, dan ada yang memberitahunya.** Bahan PWA-nya sudah lengkap sejak lama dan tidak berguna sama sekali, karena tidak ada satu pun ajakan memasangnya. Sekalian ketemu bug yang gejalanya jauh dari penyebabnya: service worker cuma didaftarkan di komponen beranda, padahal alur pertama di HP baru tidak lewat sana — jadi aplikasinya bisa dipakai seharian tanpa pernah jadi bisa dibuka luring
- [x] **Berkas `.apk` lewat GitHub Actions.** PWA yang sama dibungkus Trusted Web Activity, plus `/.well-known/assetlinks.json` yang dibaca dari env — sengaja 404 selama belum disetel, karena nilai contoh membuat Android gagal tanpa gejala
